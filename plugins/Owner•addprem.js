const handler = async (m, {conn, text, usedPrefix, command}) => {
  ...
  const textpremERROR = `🚩 Ingrese el tag del egoista que quieras agregar como egoista premium`;
  ...
  const ERROR = `🚩 Ese egoista no está en mi base de datos!`;
  ...
  if (command == 'addprem' || command == 'userpremium') {
    ...
    const textprem1 = `*🎟️ Nuevo egoista Premium!!!*\n\n*✨ User: ${name}*\n*🕐 Tiempo: ${txt} hora(s)*\n*📉 Restante: ${timeLeft} segundos*`;
  ...
  if (command == 'addprem2' || command == 'userpremium2') {
    ...
    const textprem2 = `*🎟️ Nuevo egoista Premium!!!*\n\n*✨ User: ${name}*\n*🕐 Tiempo: ${txt} día(s)*\n*📉 Restante: ${timeLeft} horas*`;
  ...
  if (command == 'addprem3' || command == 'userpremium3') {
    ...
    formatTime(user.premiumTime - now).then((timeleft) => {
      const textprem3 = `*🎟️ Nuevo egoista Premium!!!*\n\n*✨ User: ${name}*\n*🕐 Tiempo: ${txt} semana(s)*\n*📉 Restante: ${timeleft}*`;
  ...
  if (command == 'addprem4' || command == 'userpremium4') {
    ...
    user.premium = true;
    formatTime(user.premiumTime - now).then((timeleft) => {
      const textprem4 = `*🎟️ Nuevo egoista Premium!!!*\n\n*✨ Usuario: ${name}*\n*🕐 Tiempo: ${txt} mes(es)*\n*📉 Restante: ${timeleft}*`;
